...
/*!
 * This sample application uses GPS data from the gllin service, which you have to start manually
 * before using this application. The file /tmp/nmeaNP is read in for the posiitons.
 *
 ...
 */
GPS::GPS(QWidget* parent) : QMainWindow(parent)
{
    ...
    m_map_control->addLayer(
        std::make_shared<LayerMapAdapter>("Custom Layer", std::make_shared<MapAdapterOSM>()));
