...
{
    LayerMapAdapter::LayerMapAdapter(const std::string& name,
                                     const std::shared_ptr<MapAdapter>& mapadapter,
    ...
    void LayerMapAdapter::mousePressEvent(const QMouseEvent* /*mouse_event*/,
    ...
    void LayerMapAdapter::draw(QPainter& painter,
                               ...
                               const int& controller_zoom) const
    {
        ...
        if(isVisible(controller_zoom) && m_mapadapter != nullptr)
        {
            ...
            else
            {
                ...
                const QSizeF tile_size_px(ImageManager::get().tileSizePx(),
                                          ImageManager::get().tileSizePx());

                // Calculate the tiles to draw.
                const int furthest_tile_left
                    = std::floor(backbuffer_rect_px.leftPx() / tile_size_px.width());
                const int furthest_tile_top
                    = std::floor(backbuffer_rect_px.topPx() / tile_size_px.height());
                const int furthest_tile_right
                    = std::floor(backbuffer_rect_px.rightPx() / tile_size_px.width());
                const int furthest_tile_bottom
                    = std::floor(backbuffer_rect_px.bottomPx() / tile_size_px.height());
                ...
                for(int i = furthest_tile_left; i <= furthest_tile_right; ++i)
                {
                    ...
                    for(int j = furthest_tile_top; j <= furthest_tile_bottom; ++j)
                    {
                        ...
                        if(m_mapadapter->isTileValid(i, j, controller_zoom))
                        {
                            ...
                            const PointWorldPx top_left_px(i * tile_size_px.width(),
                                                           j * tile_size_px.height());

                            // Draw the tile.
                            painter.drawPixmap(top_left_px.rawPoint(),
                                               ImageManager::get().getImage(
                                                   m_mapadapter->tileQuery(i, j, controller_zoom)));
                        }
                    }
                }
                ...
                for(int i = prefetch_tile_left; i <= prefetch_tile_right; ++i)
                {
                    ...
                    {
                        // Prefetch the tile.
                        ImageManager::get().prefetchImage(
                            m_mapadapter->tileQuery(i, prefetch_tile_top, controller_zoom));
                    }
                    ...
                    if(m_mapadapter->isTileValid(i, prefetch_tile_bottom, controller_zoom))
                    {
                        // Prefetch the tile.
                        ImageManager::get().prefetchImage(
                            m_mapadapter->tileQuery(i, prefetch_tile_bottom, controller_zoom));
                    }
                }
                ...
                for(int j = prefetch_tile_top; j <= prefetch_tile_bottom; ++j)
                {
                    ...
                    if(m_mapadapter->isTileValid(prefetch_tile_left, j, controller_zoom))
                    {
                        // Prefetch the tile.
                        ImageManager::get().prefetchImage(
                            m_mapadapter->tileQuery(prefetch_tile_left, j, controller_zoom));
                    }
                    ...
                    if(m_mapadapter->isTileValid(prefetch_tile_right, j, controller_zoom))
                    {
                        // Prefetch the tile.
                        ImageManager::get().prefetchImage(
                            m_mapadapter->tileQuery(prefetch_tile_right, j, controller_zoom));
                    }
                }
            }
        }
