...
namespace qmapcontrol
{
    ...
    bool MapAdapter::isTileValid(const int& x, const int& y, const int& controller_zoom) const
    {
        ...
        if(m_epsg_projections.find(static_cast<projection::EPSG>(projection::get().epsg()))
           == m_epsg_projections.end())
        {
        ...
        // Else check the zoom is supported (if min <= max).
                ...
                && (toAdapterZoom(controller_zoom) < m_adapter_zoom_minimum
                    || toAdapterZoom(controller_zoom) > m_adapter_zoom_maximum))
        {
        ...
        else if(x >= 0 && x < projection::get().tilesX(controller_zoom) && y >= 0
                && y < projection::get().tilesY(controller_zoom))
        {
