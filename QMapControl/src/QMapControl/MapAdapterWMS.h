...
namespace qmapcontrol
{
    ...
    public:
         ...
         * mapadapter(std::make_shared<MapAdapterWMS>("http://www2.demis.nl/wms/wms.asp?wms=WorldMap[...]&BBOX=%1,%2,%3,%4&WIDTH=%5&HEIGHT=%5&TRANSPARENT=TRUE",
         * ...)); The placeholders %1, %2, %3, %4 creates the bounding box. %5 is where you specify
         * the tile size in pixels.
         * @param base_url The base url of the map server.
         * @param epsg_projections The supported EPSG projections.
         * @param parent QObject parent ownership.
         */
        MapAdapterWMS(const QUrl& base_url,
                      const std::set<projection::EPSG>& epsg_projections,
                      QObject* parent = 0);

        //! Disable copy constructor.
        /// MapAdapterWMS(const MapAdapterWMS&) = delete; @todo re-add once MSVC supports
        /// default/delete syntax.

        //! Disable copy assignment.
        /// MapAdapterWMS& operator=(const MapAdapterWMS&) = delete; @todo re-add once MSVC supports
        /// default/delete syntax.

        //! Destructor.
        virtual ~MapAdapterWMS() {
    ...
    protected:
         ...
         */
        virtual QString
        getBBox(const qreal& x1, const qreal& y1, const qreal& x2, const qreal& y2) const;

    private:
        //! Disable copy constructor.
        MapAdapterWMS(const MapAdapterWMS&); /// @todo remove once MSVC supports default/delete
                                             /// syntax.

        //! Disable copy assignment.
        MapAdapterWMS& operator=(const MapAdapterWMS&); /// @todo remove once MSVC supports
                                                        /// default/delete syntax.
    };
}
