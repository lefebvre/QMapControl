...
{
    GeometryPolygonImage::GeometryPolygonImage(const PointWorldCoord& top_left_coord,
                                               const PointWorldCoord& bottom_right_coord,
                                               const std::string& filename,
                                               const int& zoom_minimum,
                                               const int& zoom_maximum)
        : GeometryPolygon(RectWorldCoord(top_left_coord, bottom_right_coord).toStdVector(),
                          zoom_minimum,
                          zoom_maximum),
    ...
    void GeometryPolygonImage::draw(QPainter& painter,
                                    ...
                                    const int& controller_zoom)
    {
        ...
        if(isVisible(controller_zoom))
        {
            ...
            if(backbuffer_rect_coord.rawRect().intersects(image_rect_coord.rawRect()))
            {
                ...
                const RectWorldPx image_rect_px(
                    projection::get().toPointWorldPx(image_rect_coord.topLeftCoord(),
                                                     controller_zoom),
                    projection::get().toPointWorldPx(image_rect_coord.bottomRightCoord(),
                                                     controller_zoom));
