...
{
    GeometryLineString::GeometryLineString(const std::vector<PointWorldCoord>& points,
    ...
    void GeometryLineString::draw(QPainter& painter,
                                  ...
                                  const int& controller_zoom)
    {
        ...
        if(isVisible(controller_zoom))
        {
            ...
            if(QPolygonF(backbuffer_rect_coord.rawRect()).intersected(polygon_line).empty()
               == false)
            {
                // Create a polygon of the points.
