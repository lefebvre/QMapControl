...
namespace qmapcontrol
{
    ...
    public:
         ...
         */
        LayerGeometry(const std::string& name,
                      const int& zoom_minimum = 0,
                      const int& zoom_maximum = 17,
                      QObject* parent = 0);

        //! Disable copy constructor.
        /// LayerGeometry(const LayerGeometry&) = delete; @todo re-add once MSVC supports
        /// default/delete syntax.

        //! Disable copy assignment.
        /// LayerGeometry& operator=(const LayerGeometry&) = delete; @todo re-add once MSVC supports
        /// default/delete syntax.
        ...
        /*!
         * Returns the Geometry objects from this Layer (Use this instead of the member variable for
         * thread-safety).
         * @param range_coord The bounding box range to limit the geometries that are fetched in
         * coordinates.
         * @return a list of geometries that are on this Layer within the bounding box range.
         */
        const std::set<std::shared_ptr<Geometry>>
        getGeometries(const RectWorldCoord& range_coord) const;

        /*!
         * Returns the Geometry QWidgets from this Layer (Use this instead of the member variable
         * for thread-safety).
         ...
         */
        bool containsGeometry(const std::shared_ptr<Geometry>& geometry,
         ...
         * @param controller_zoom The current controller zoom.
         */
        void mousePressEvent(const QMouseEvent* mouse_event,
                             const PointWorldCoord& mouse_point_coord,
                             const int& controller_zoom) const final;
         ...
         * @param backbuffer_rect_px Only draw map tiles/geometries that are contained in the
         * backbuffer rect (pixels).
         * @param controller_zoom The current controller zoom.
         */
        ...
        /*!
         * Moves any geometries that represent a widget, as these are not drawn to the actually
         * pixmap.
